...
static shiftBF *sbf;
static double swrong[SET_NUM+1];
static uint32_t sum_sw;

static KBF *kbf;
static double kwrong[SET_NUM+1];
static uint32_t sum_kw;

static DJBF *djbf;
static double djwrong[SET_NUM+1];
...
void sbf_test(){
    ...
    for (uint8_t g = 0; g <= SET_NUM; ++g){
        ...
        for (uint32_t i = 0; i < n; ++i){
            ...
            if (r != g) ++wrong; if (g == SET_NUM && r == -1) --wrong;
		}
		if (n != 0)
...
void write(){
	FILE *outfile = fopen("../result/WA_rate.txt", "a");
	uint32_t s = 0;
	for (uint8_t g = 0; g < SET_NUM; ++g)
	  s += length[g].size();
	uint32_t outsize = length[SET_NUM].size();	s += outsize;
